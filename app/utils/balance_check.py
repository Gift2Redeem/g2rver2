...
class BalanceCheck():
    def card_balance(self, card):
        card_type=card["type"]
        res = ''
        if card_type =='Target':
            res=self.get_target_card_balance(card)
        elif card_type =='BestBuy':
            res=self.get_best_buy_card_balance(card)
        elif card_type =='OldNavy':
            res=self.get_oldnavy_card_balance(card)
        elif card_type =='Toysrus': 
            res=self.get_toys_r_us_card_balance(card)
        return res
    ...
    def get_best_buy_card_balance(self, card):
        ...
        try:
            u = urllib.parse.urlopen("https://www.bestbuybusiness.com/bbfb/en/US/adirect/bestbuy?cmd=BBFBGiftCardBalance", data)
            html = u.read()
    ...
    def get_target_card_balance(self, card):
        ...
        try:
            u = urllib.parse.urlopen("https://www-secure.target.com/GuestGCCheckGiftCardBalCmd", data)
    ...
    def get_oldnavy_card_balance(self, card):
        try:
    ...
    def get_toys_r_us_card_balance(self, card):
        try:
            browser = mechanicalsoup.Browser()
